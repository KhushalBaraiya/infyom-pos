...
import { currencySymbolHandling, decimalValidate, getDecimalPlaces, getFormattedMessage } from "../../sharedMethod";
...
const PurchaseTable = (props) => {
    ...
    useEffect(() => {
        if (!singleProduct?.in_warehouse_stock) return;
    
        const foundItem = singleProduct.in_warehouse_stock.find(
            (item) => item.name === warehouseName
        );
    
        if (foundItem) {
            singleProduct.stock = foundItem.total_quantity;
        } else {
            singleProduct.stock = 0;
        }
    }, [singleProduct?.in_warehouse_stock, warehouseName]);
...
const mapStateToProps = (state) => {
    const { productUnits, frontSetting, allConfigData , settings } = state;
    return { productUnits, frontSetting, allConfigData , settings};
};
