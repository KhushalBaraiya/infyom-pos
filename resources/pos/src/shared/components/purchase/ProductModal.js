...
const ProductModal = ( props ) => {
    ...
    useEffect( () => {
        ...
        setNetUnitCost( netUnitCost ? ( netUnitCost ).toFixed( decimalPlaces ) : parseFloat( product.net_unit_cost ).toFixed( decimalPlaces ) );
   ...
   const onChangePrice = (e) => {
    ...
    const onChangeDiscount = (e) => {
        ...
        // % discount max 100
        if (discountType.value === 1 && parseFloat(value) > 100) {
            setErrors(prev => ({
                ...prev,
                discountValue: getFormattedMessage(
                    "globally.discount-length.validate.label"
                ),
            }));
            return;
        }
        ...
        // fixed discount <= cost
        if (discountType.value === 2 && parseFloat(value) > netUnitCost) {
            setErrors(prev => ({
                ...prev,
                discountValue: getFormattedMessage(
                    "globally.discount-cost-length.validate.label"
                ),
            }));
            return;
        }
