...
export const fetchSalesReturn =
    ...
    export const fetchCustomerSalesReturn =
    (filter = {}, isLoading = true) =>
    async (dispatch) => {
        if (isLoading) {
            dispatch(setLoading(true));
        ...
        const admin = true;
        let url = 'customer/purchases-return';
        if (
            !_.isEmpty(filter) &&
            (filter.page ||
                filter.pageSize ||
...
export const deleteSaleReturn = (userId, setNotDeletedItemModal, clearSelectedDeleteItem) => async (dispatch) => {
    await apiConfig
        .delete(apiBaseURL.SALE_RETURN, { data: userId })
        .then((response) => {
