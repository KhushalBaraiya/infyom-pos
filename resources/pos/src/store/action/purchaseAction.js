...
export const fetchCustomerPurchases =
    (filter = {}, isLoading = true) =>
    async (dispatch) => {
        if (isLoading) {
            dispatch(setLoading(true));
        }
        let url = apiBaseURL.CUSTOMER_PURCHASE_PRODUCT;
        if (
            !_.isEmpty(filter) &&
            (filter.page ||
                filter.pageSize ||
        ...
        apiConfig
            ...
            .then((response) => {
                dispatch({
                    type: purchaseActionType.FETCH_CUSTOMER_PURCHASES,
                    payload: response.data.data,
                });
                dispatch(
                    setTotalRecord(
...
export const deletePurchase = (purchaseId, setNotDeletedItemModal, clearSelectedDeleteItem) => async (dispatch) => {
    apiConfig
        .delete(apiBaseURL.PURCHASES, { data: purchaseId })
        .then((response) => {
