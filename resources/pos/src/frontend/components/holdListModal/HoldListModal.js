    ...
    const getHoldData = () => {
        ...
        if ( items !== undefined ) {
            const arrat = items?.map( ( items ) => {
                const quantity = items.product?.stocks?.find(item => item?.warehouse_id === editholdListData?.attributes?.warehouse_id)?.quantity || 0;
                const prepareArray = {
                    ...
                    product_price: Number( items.product_price ),
                    original_product_price: Number( items.product.product_price ),
                    product_wholesale_price: items.product.product_wholesale_price,
                    product_special_price: items.product.product_special_price,
                    product_price_type: items.price_group,
                    fix_net_unit: Number( items.product_price ),
                    net_unit_cost: Number( items.product_price ),
                    net_unit_price: Number( items.product_price ),
                    ...
                    quantity: quantity < items.quantity ? quantity : items.quantity,
            ...
            setCartItemValue( {
                ...
                discount_type: editholdListData?.attributes?.discount_type ?? discountType.FIXED,
                discount_value: editholdListData?.attributes?.discount_value ?? 0,
            } )
                                ...
                                {holdListData && holdListData.length > 0 && holdListData.map( ( items, index ) => {
                                    return (
                                        <tr key={index}>
                                            ...
                                            <td>
                                                <FontAwesomeIcon onClick={() => {onConfirm(items.id, true)}} cursor={"pointer"} className={"me-3 edit"} icon={faPenToSquare} />
