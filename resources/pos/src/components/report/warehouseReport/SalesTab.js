...
const SalesTab = (props) => {
    ...
    useEffect(() => {
        if (isWarehouseValue === true) {
            saleExcelAction(warehouseValue.value, setIsWarehouseValue, currentfilter);
        }
    }, [isWarehouseValue]);
...
const mapStateToProps = (state) => {
    const { isLoading, totalRecord, sales, frontSetting, settings } = state;
    return { isLoading, totalRecord, sales, frontSetting, settings };
};
