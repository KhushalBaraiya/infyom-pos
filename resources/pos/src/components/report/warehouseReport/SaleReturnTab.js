...
const SaleReturnTab = (props) => {
    ...
    useEffect(() => {
        if (isWarehouseValue === true) {
            saleReturnExcelAction(warehouseValue.value, setIsWarehouseValue, currentFilter);
        }
    }, [isWarehouseValue]);
...
const mapStateToProps = (state) => {
    const { isLoading, totalRecord, salesReturn, frontSetting, settings } = state;
    return { isLoading, totalRecord, salesReturn, frontSetting, settings };
};
