...
import {getDecimalPlaces, getFormattedMessage, placeholderText} from '../../../shared/sharedMethod';
...
const StockDetails = (props) => {
    const {stockDetailsWarehouseAction, stockWarehouse, allConfigData, settings} = props;
                        ...
                        {stockWarehouse && stockWarehouse.map((warehouse, index) => {
                            return (
                                <tr key={index}>
                                    ...
                                    <td>{parseFloat(warehouse.quantity).toFixed(decimalPlaces)} {stockWarehouse[0] && stockWarehouse[0].product.product_unit === "1" && "Pc" || stockWarehouse[0].product.product_unit === "2" && "M" || stockWarehouse[0].product.product_unit === "3" && "Kg"}</td>
                                </tr>
                            )
                        })}
...
const mapStateToProps = (state) => {
    const {stockWarehouse, allConfigData, settings} = state;
    return {stockWarehouse, allConfigData, settings}
};
