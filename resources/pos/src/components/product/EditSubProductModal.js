...
import { decimalValidate, generateBarCode, getDecimalPlaces, getFormattedMessage, getFormattedOptions, placeholderText } from "../../shared/sharedMethod";
...
const EditSubProductModal = (props) => {
    ...
    const { frontSetting,settings } = useSelector((state) => state);
    ...
    useEffect(() => {
        if (show) {
            ...
            setFormInput((prev) => ({
                ...
                product_wholesale_price: productData.product_wholesale_price,
                product_special_price: productData.product_special_price,
    ...
    const decimalPlace = getDecimalPlaces(settings);
    ...
    const onProductDataChange = (e) => {
        ...
        // Decimal fields
        const decimalFields = ["product_cost", "product_price", "product_wholesale_price", "product_special_price", "order_tax"];
        ...
        if (decimalFields.includes(name)) {
            ...
            if (parts[1]?.length > decimalPlace) return;

            // Order tax must be ≤ 100
            if (name === "order_tax" && parseFloat(value) > 100) {
                return;
            }

            if ((name === 'product_wholesale_price' || name === 'product_special_price') && formInput.product_price && parseFloat(value) > parseFloat(formInput.product_price)) {
                return;
            }

            if (name === 'product_price') {
                const typedRetail = parseFloat(value);
                if (formInput.product_wholesale_price && typedRetail < parseFloat(formInput.product_wholesale_price)) {
                    return;
                }
                if (formInput.product_special_price && typedRetail < parseFloat(formInput.product_special_price)) {
                    return;
                }
            }
        }
    ...
    const prepareFormData = (commonData, formInput) => {
        ...
        formData.append('product_wholesale_price', formInput.product_wholesale_price);
        formData.append('product_special_price', formInput.product_special_price);
                                 ...
                                 <div className="col-md-3 mb-3">
                                     ...
                                     <InputGroup>
                                         <input
                                             ...
                                             value={
                                                 formInput.product_wholesale_price
                                             }
                                         />
                                         <InputGroup.Text>
                                             {frontSetting.value &&
                                     ...
                                     <span className="text-danger d-block fw-400 fs-small mt-2">
                                         {errors["product_wholesale_price"]
                                             ? errors["product_wholesale_price"]
                                             : null}
                                     </span>
                                 </div>
                                 <div className="col-md-3 mb-3">
                                     ...
                                     <InputGroup>
                                         <input
                                             ...
                                             value={
                                                 formInput.product_special_price
                                             }
                                         />
                                         <InputGroup.Text>
                                             {frontSetting.value &&
