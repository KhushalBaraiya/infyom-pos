...
const EditProduct = (props) => {
    ...
    const subProduct =
        singleProduct.length >= 1 && singleProduct[0]?.attributes?.products[0];
    const getSaleUnit =
        subProduct && subProduct.sale_unit_name
            ? {
    ...
    const getPurchaseUnit =
        subProduct && subProduct.purchase_unit_name
            ? {
    ...
    const getProductUnit =
        mainProductItemsValue &&
        base.filter(
            (fill) =>
                Number(fill?.id) ===
                Number(mainProductItemsValue[0]?.product_unit)
        );
