...
const TransferForm = ( props ) => {
    ...
    useEffect(() => {
        if (singleTransfer && !transferValueUpdated) {
            setTransferValue({
                ...
                tax_rate: parseFloat(singleTransfer.tax_rate).toFixed(decimalPlaces),
                tax_amount: parseFloat(singleTransfer.tax_amount).toFixed(decimalPlaces),
                discount: parseFloat(singleTransfer.discount).toFixed(decimalPlaces),
                shipping: parseFloat(singleTransfer.shipping).toFixed(decimalPlaces),
    ...
    const handleValidation = () => {
        ...
        } else if (validateFiscalDate(transferValue.date)) {
            errorss[ 'date' ] = validateFiscalDate(transferValue.date);
    ...
    return (
        <div className='card'>
            <TabTitle title={placeholderText(singleTransfer ? "transfer.edit.title" : "transfer.create.title")} />
                ...
                <div className='row'>
                    <div className='col-md-4'>
                        ...
                        <div className='position-relative'>
                            <ReactDatePicker onChangeDate={handleCallback} {...(!isFiscalYearEnabled && { newStartDate: transferValue.date })} FixedFiscalYearDate={true} />
                        </div>
...
const mapStateToProps = ( state ) => {
    const { products, frontSetting, allConfigData , settings, isLoading } = state;
    return { customProducts: prepareTransferArray( products ), products, frontSetting, allConfigData , settings, isLoading}
};
