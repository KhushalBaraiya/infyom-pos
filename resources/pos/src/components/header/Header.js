...
import { fetchFiscalYear, updateFiscalYear } from '../../store/action/FiscalYearAction.js';
import Select from 'react-select';
import { fetchSetting } from '../../store/action/settingAction.js';

const Header = (props) => {
    const { logoutAction, newRoutes, updateLanguage, selectedLanguage, productQuantityReportAction, productQuantityReport, updateFiscalYear } = props;
    ...
    useEffect(() => {
        if (!fiscalYears.length > 0) dispatch(fetchFiscalYear({ page: 1, pageSize: 100 }));
        if(!settings || !settings.attributes) dispatch(fetchSetting({}))
    }, [])

    useEffect(() => {
        if (fiscalYears.length > 0) {
            const activeFiscalYear = fiscalYears.find(fy => fy.attributes?.is_active || fy.is_active);
            if (activeFiscalYear) {
    ...
    return (
        <Navbar collapseOnSelect expand='lg' className='align-items-stretch ms-auto py-1 h-auto'>
            <div className='d-flex align-items-stretch justify-content-center'>
                <Nav className='align-items-stretch justify-content-between flex-row'>
                    <ul className='nav align-items-center md-flex-wrap sm-justify-content-end md-justify-content-between md-gap-1 sm-gap-2'>
                        {!window.location.pathname.includes('report') || !window.location.pathname.includes('settings') && <li className="px-sm-3 px-2 d-flex align-items-center">
                            ...
                            {(fiscalYears.length > 0 && isFiscalYearEnabled && !allConfigData?.permissions?.includes('manage_customer_purchases')) && <li className="px-sm-3 px-2">
                                <div className='md-m-0 m-2'>
                                    <Select
                                        ...
                                        onChange={(selected) => {
                                            ...
                                            updateFiscalYear({ id: selected.value, is_active: true });
                                        }}
                                        options={fiscalYears?.map(fy => ({
                                            value: fy.id,
                                            label: fy.attributes?.name || fy.name
                                        }))}
                                        placeholder="Select Fiscal Year"
                                        isSearchable={false}
                                        isDisabled={!allConfigData?.permissions?.includes('manage_setting') ? true : false}
                                    />
                                </div>
                            </li>}
                        <li className='ms-4 md-ms-0'>
...
export default connect(mapStateToProps, { logoutAction, updateLanguage, productQuantityReportAction, updateFiscalYear })(Header);
