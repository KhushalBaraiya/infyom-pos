    ...
    <div>
        ...
        <table width="100%">
            <tr>
                ...
                <!-- Quotation Info (left in RTL) -->
                <td width="30%" style="text-align: right;">
                    <table style="width:100%; border-collapse: collapse;">
                        <tr>
                            <td><b>{{ __('messages.pdf.date') }}</b> :
                                {{ getFormattedDate($quotation->created_at) }}
                            </td>
                        </tr>
                        <tr>
                            <td><b>{{ __('messages.pdf.number') }}</b> : {{ $quotation->reference_code }}</td>
                        </tr>
                        <tr>
                ...
                <td style="width:48%; vertical-align: top;">
                    <table class="table">
                        ...
                        <tbody>
                            <tr>
                                <td>
                                    <b>{{ __('messages.pdf.name') }}</b> : {{ $quotation->customer->name ?? 'N/A' }}
                                    <br>
                                    <b>{{ __('messages.pdf.phone') }}</b> : {{ $quotation->customer->phone ?? 'N/A' }}
                                    <br>
                                    <b>{{ __('messages.pdf.address') }}</b> :
                                    @if (
                                        !empty($quotation->customer->address) ||
                                            !empty($quotation->customer->city) ||
                                            !empty($quotation->customer->country))
                                    ...
                                    @endif
                                    <br>
                                    <b>{{ __('messages.pdf.email') }}</b> : {{ $quotation->customer->email ?? 'N/A' }}
                                </td>
                            </tr>
                        </tbody>
                    </table>
        ...
        <table class="table" style="margin-top: 40px;">
            ...
            <tbody>
                @foreach ($quotation->quotationItems as $quotationItem)
                    <tr>
                        <td style="text-align: right;">{{ $quotationItem->product->name }}</td>
                        <td class="text-center icon-style">
                            {{ currencyAlignment((float) $quotationItem->net_unit_price) }}</td>
                        <td class="text-center">{{ $quotationItem->quantity }}</td>
                        <td class="text-center icon-style">
                            {{ currencyAlignment((float) $quotationItem->discount_amount) }}</td>
                        <td class="text-center icon-style">
                            {{ currencyAlignment((float) $quotationItem->tax_amount) }}</td>
                        <td class="number-align icon-style">
                            {{ currencyAlignment((float) $quotationItem->sub_total) }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <table class="table" style="width:40%; margin-top: 20px; float: left;">
            <tbody>
                <tr>
                    ...
                    <td class="number-align icon-style">
                        {{ currencyAlignment((float) $quotation->tax_amount) }}
                    </td>
                </tr>
                <tr>
                    <td><strong>{{ __('messages.pdf.discount') }}</strong> :</td>
                    <td class="number-align icon-style">
                        {{ currencyAlignment((float) $quotation->discount) }}
                    </td>
                </tr>
                <tr>
                    <td><strong>{{ __('messages.pdf.shipping') }}</strong> :</td>
                    <td class="number-align icon-style">
                        {{ currencyAlignment((float) $quotation->shipping) }}
                    </td>
                </tr>
                <tr>
                    <td><strong>{{ __('messages.pdf.total') }}</strong> :</td>
                    <td class="number-align icon-style">
                        <b>{{ currencyAlignment((float) $quotation->grand_total) }}</b>
                    </td>
                </tr>
            </tbody>
        </table>
