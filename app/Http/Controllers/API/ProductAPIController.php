...
class ProductAPIController extends AppBaseController
{
    ...
    public function index(Request $request): ProductCollection
    {
        ...
        $isPosPage = (int) $perPage === 20;
        $showPosStockProduct = filter_var(
            getTenantSettingValue('show_pos_stock_product'),
            FILTER_VALIDATE_BOOLEAN
        );

        $onlyInStock = $isPosPage && ! $showPosStockProduct;
        ...
        if ($request->get('warehouse_id') && $request->get('warehouse_id') != 'null') {
            ...
            $products->whereHas('stock', function ($q) use ($warehouseId, $onlyInStock) {
                $q->where('manage_stocks.warehouse_id', $warehouseId);

                if ($onlyInStock) {
                    $q->where('manage_stocks.quantity', '>', 0);
                }
            })->with([
                'stock' => function (HasOne $query) use ($warehouseId, $onlyInStock) {
            ...
            $products->whereHas('stock', function ($q) {
                $q->where('manage_stocks.quantity', '>', 0);
            });
        }
