...
class ReportAPIController extends AppBaseController
{
    ...
    public function getSupplierReport(Request $request): JsonResponse
    {
        ...
        $purchaseFiscalYearFilter = function ($query) use ($fiscalYear) {
            if ($fiscalYear) {
        ...
        $suppliers = $this->supplierRepository
            ->withCount(['purchases' => $purchaseFiscalYearFilter])
            ->with(['purchases' => $purchaseFiscalYearFilter])
            ->paginate($perPage);
    ...
    public function getProfitLossReport(Request $request)
    {
        ...
        if (!$hasCustomDate && isFiscalYearFilterEnabled()) {
            $fiscalYearId = $request->input('filter.fiscal_year_id', $request->get('fiscal_year_id'));
        ...
        $hasDateRange = filled($startDate) && filled($endDate) && $startDate !== 'null' && $endDate !== 'null';
        $applyDateRange = function ($query, string $column) use ($hasDateRange, $startDate, $endDate) {
            return $hasDateRange ? $query->whereBetween($column, [$startDate, $endDate]) : $query;
        };

        $data['sales'] = $applyDateRange(Sale::query(), 'date')->sum('grand_total');
        $data['purchase_returns'] = $applyDateRange(PurchaseReturn::whereHas('warehouse'), 'date')->sum('grand_total');
        $data['purchases'] = $applyDateRange(Purchase::whereHas('warehouse'), 'date')->sum('grand_total') - $data['purchase_returns'];
        $data['sale_returns'] = $applyDateRange(SaleReturn::whereHas('warehouse'), 'date')->sum('grand_total');
        $data['expenses'] = $applyDateRange(Expense::whereHas('warehouse'), 'date')->sum('amount');
        $data['sales_payment_amount'] = $applyDateRange(SalesPayment::whereHas('sale'), 'payment_date')->sum('amount');
        ...
        $sales = $applyDateRange(Sale::query(), 'date')->with('saleItems.product')->get();

        $allSaleReturnsItems = SaleReturnItem::withWhereHas('product')
            ->join('sales_return', 'sales_return.id', '=', 'sale_return_items.sale_return_id')
            ->join('sales', 'sales.id', '=', 'sales_return.sale_id');

        if ($hasDateRange) {
            $allSaleReturnsItems->whereBetween('sales.date', [$startDate, $endDate]);
        }

        $allSaleReturnsItems = $allSaleReturnsItems
    ...
    public function getCustomerReport(Request $request)
    {
        ...
        $salesFiscalYearFilter = function ($query) use ($fiscalYear) {
            if ($fiscalYear) {
                $query->whereDate('date', '>=', $fiscalYear->start_date)
                    ->whereDate('date', '<=', $fiscalYear->end_date);
            }
        ...
        $customers = $this->customerRepository
            ->withCount(['sales' => $salesFiscalYearFilter])
            ->with(['sales' => $salesFiscalYearFilter])
            ->paginate($perPage);
