...
class UpdateSupplierRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $rules['email'] = 'required|email|unique:suppliers,email,' . $this->route('supplier') . ',id,tenant_id,' . Auth::user()->tenant_id;

        if (!$settings['supplier_email_required']) {
            $rules['email'] = 'nullable|email|unique:suppliers,email,' . $this->route('supplier') . ',id,tenant_id,' . Auth::user()->tenant_id;
        }
