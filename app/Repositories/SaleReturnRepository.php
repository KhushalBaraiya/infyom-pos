...
class SaleReturnRepository extends BaseRepository
{
    ...
    public function storeSaleReturn($input): SaleReturn
    {
        try {
            ...
            $walletRefundAmount = (int) $input['wallet_refund_amount'] ?? 0;
            $customerWallet = $sale->customer?->wallet ?? null;
            if (isset($input['is_wallet_refund']) && $input['is_wallet_refund'] && $walletRefundAmount > 0 && $customerWallet) {
                // $customerWallet->update(['amount' => $customerWallet->amount + $walletRefundAmount]);
                $customerWallet->increment('amount', $walletRefundAmount);
                ...
                $sale->update(['wallet_refund_amount' => $walletRefundAmount]);
            }
            ...
            foreach ($input['sale_return_items'] as $purchaseItem) {
                $product = ManageStock::whereWarehouseId($input['warehouse_id'])
                    ...
                    ->first();
                $saleExist = SaleItem::where('product_id', $purchaseItem['product_id'])->whereHas(
                    'sale',
                    function (Builder $q) use ($input) {
                        $q->where('customer_id', $input['customer_id'])->where(
                            'warehouse_id',
                            $input['warehouse_id']
    ...
    public function storeSaleReturnItems($saleReturn, $input)
    {
        foreach ($input['sale_return_items'] as $saleReturnItem) {
            ...
            $item = $this->calculationSaleReturnItems($saleReturnItem, $input['sale_id'] ?? null);
     ...
     * @return mixed
     */
    public function calculationSaleReturnItems($saleReturnItem, $saleId = null)
    {
        $saleReturnItem = $this->resolveSaleReturnItemPrice($saleReturnItem, $saleId);
    ...
    /**
     * Resolve the unit price for a sale return item.
     *
     * Prefers the original sale item price when a sale ID is available so returns
     * remain consistent with the price actually sold.
     */
    protected function resolveSaleReturnItemPrice(array $saleReturnItem, $saleId = null): array
    {
        ...
        if ($saleId) {
            $saleItem = SaleItem::where('sale_id', $saleId)
                ->where('product_id', $saleReturnItem['product_id'])
                ->first();
        }

        $requestedPrice = array_key_exists('product_price', $saleReturnItem) ? $saleReturnItem['product_price'] : null;
        $requestedPrice = ($requestedPrice === '' || $requestedPrice === null) ? null : (float) $requestedPrice;
        $priceGroup = $saleReturnItem['price_group'] ?? ($saleItem?->price_group ?? null);
        ...
        if ($priceGroup === null || $priceGroup === '') {
            ...
            if ($referencePrice === null && $saleItem) {
                $referencePrice = (float) $saleItem->product_price;
            }

            if ($referencePrice !== null) {
                if ($priceMatches($referencePrice, $product->product_price)) {
                    $priceGroup = SaleReturnItem::PRICE_GROUP_PRODUCT_PRICE;
                } elseif ($priceMatches($referencePrice, $product->product_wholesale_price)) {
                    $priceGroup = SaleReturnItem::PRICE_GROUP_WHOLESALE_PRICE;
                } elseif ($priceMatches($referencePrice, $product->product_special_price)) {
                    $priceGroup = SaleReturnItem::PRICE_GROUP_SPECIAL_PRICE;
                } else {
                    $priceGroup = SaleReturnItem::PRICE_GROUP_CUSTOM_PRICE;
                }
            } else {
                $priceGroup = SaleReturnItem::PRICE_GROUP_PRODUCT_PRICE;
            }
        ...
        if (! in_array($priceGroup, [
            SaleReturnItem::PRICE_GROUP_PRODUCT_PRICE,
            SaleReturnItem::PRICE_GROUP_WHOLESALE_PRICE,
            SaleReturnItem::PRICE_GROUP_SPECIAL_PRICE,
            SaleReturnItem::PRICE_GROUP_CUSTOM_PRICE,
        ], true)) {
            throw new UnprocessableEntityHttpException('Invalid price group selected.');
        }
        $saleReturnItem['price_group'] = $priceGroup;
        switch ($priceGroup) {
            case SaleReturnItem::PRICE_GROUP_PRODUCT_PRICE:
                if ($requestedPrice === null) {
                    $saleReturnItem['product_price'] = $saleItem ? (float) $saleItem->product_price : (float) $product->product_price;
                } else {
                    $saleReturnItem['product_price'] = $requestedPrice;
                }
                break;
            case SaleReturnItem::PRICE_GROUP_WHOLESALE_PRICE:
                if ($requestedPrice === null) {
                    if ($saleItem && $saleItem->product_price !== null) {
                        $saleReturnItem['product_price'] = (float) $saleItem->product_price;
                    } elseif ($product->product_wholesale_price !== null && $product->product_wholesale_price !== '') {
                        $saleReturnItem['product_price'] = (float) $product->product_wholesale_price;
                    } else {
                        throw new UnprocessableEntityHttpException('Wholesale price is not set for this product.');
                    }
                }
                break;
            case SaleReturnItem::PRICE_GROUP_SPECIAL_PRICE:
                if ($requestedPrice === null) {
                    if ($saleItem && $saleItem->product_price !== null) {
                        $saleReturnItem['product_price'] = (float) $saleItem->product_price;
                    } elseif ($product->product_special_price !== null && $product->product_special_price !== '') {
                        $saleReturnItem['product_price'] = (float) $product->product_special_price;
                    } else {
                        throw new UnprocessableEntityHttpException('Special price is not set for this product.');
                    }
                }
                break;
            case SaleReturnItem::PRICE_GROUP_CUSTOM_PRICE:
                if ($requestedPrice === null) {
                    if ($saleItem && $saleItem->product_price !== null) {
                        $saleReturnItem['product_price'] = (float) $saleItem->product_price;
                    } else {
    ...
    public function updateSaleReturn($input, $id)
    {
        try {
            DB::beginTransaction();
            $saleReturn = SaleReturn::with('sale', 'customer.wallet')->findOrFail($id);

            $isWalletRefund = $input['is_wallet_refund'] ?? false;
            $newWalletAmount = isset($input['wallet_refund_amount']) ? (float) $input['wallet_refund_amount'] : 0;
            $newWalletAmount = $isWalletRefund ? $newWalletAmount : 0;
            $sale = $saleReturn->sale;
            $customerWallet = $saleReturn->customer?->wallet ?? null;
            $oldWalletAmount = (float) ($sale->wallet_refund_amount ?? 0);
            $difference = $newWalletAmount - $oldWalletAmount;
            $walletPaymentMethod = PaymentMethod::where('type', PaymentMethod::CUSTOMER_WALLET)->first();
            if ($difference > 0) {
                $customerWallet->increment('amount', $difference);
                $customerWallet->transactions()->create([
                    'direction' => CustomerWalletTransaction::DIRECTION_CREDIT,
                    'payment_method_id' => $walletPaymentMethod->id,
                    'amount' => $difference,
                    'transaction_type' => CustomerWalletTransaction::TRANSACTION_TYPE_REFUND,
                    'status' => CustomerWalletTransaction::STATUS_APPROVED,
                    'notes' => 'Sale return updated #' . $saleReturn->id,
                ]);
            } elseif ($difference < 0) {
                $debitAmount = abs($difference);
                if ($customerWallet->amount < $debitAmount) {
                    throw new UnprocessableEntityHttpException(
                        'Insufficient wallet balance. Available balance: ' . $customerWallet->amount
                    );
                }
                $customerWallet->decrement('amount', $debitAmount);
                $customerWallet->transactions()->create([
            ...
            $sale->update([
                'wallet_refund_amount' => $newWalletAmount
            ]);
            ...
            foreach ($input['sale_return_items'] as $key => $saleReturnItem) {
                ...
                $this->updateItem($saleReturnItemArray, $input['warehouse_id'], $input['sale_id'] ?? null);
    ...
    public function updateItem($saleReturnItem, $warehouseId, $saleId = null): bool
    {
        try {
            $saleReturnItem = $this->calculationSaleReturnItems($saleReturnItem, $saleId);
